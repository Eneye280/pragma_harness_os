    ...
    <div className="fixed inset-0 z-40" role="presentation">
      ...
      >
        <div className="flex items-center gap-2 border-b border-hairline px-3 py-3">
          <span id="sessions-title" className="text-[12px] font-semibold uppercase tracking-[0.16em] text-zinc-400">
            Sesiones
          </span>
          <span className="rounded-full bg-zinc-800 px-1.5 py-[1px] text-[12px] text-zinc-400">{sessions.length}</span>
          <button
            type="button"
            onClick={onNew}
            className="ml-auto rounded-control border border-harness/40 bg-harness/10 px-2 py-1 text-[12px] text-harness-soft transition-colors hover:bg-harness/20"
          >
            Nueva
          </button>
          <button
        ...
        <ul className="flex-1 space-y-1 overflow-y-auto p-2">
          {loading ? (
            <li className="px-2 py-3 text-[12px] text-zinc-500">cargando…</li>
          ) : visible.length === 0 ? (
            <li className="px-2 py-3 text-[12px] text-zinc-500">sin sesiones</li>
          ) : (
            visible.map((session) => {
                ...
                >
                  {isEditing ? (
                    ...
                    <button type="button" onClick={() => onOpen(session.id)} className="block w-full text-left">
                      <span className="flex items-center gap-1.5">
                        ...
                        {isCurrent ? <span className="shrink-0 text-[12px] text-harness-soft">activa</span> : null}
                        {isRunning ? <span className="shrink-0 text-[12px] text-harness-soft">corriendo</span> : null}
                      </span>
                  ...
                  <div className="mt-1 flex items-center gap-1">
                    <button
                      ...
                      className="rounded-control px-1.5 py-0.5 text-[12px] text-zinc-500 transition-colors hover:text-zinc-200"
                    >
                      renombrar
                    </button>
                    <button
                      ...
                      className="rounded-control px-1.5 py-0.5 text-[12px] text-zinc-500 transition-colors hover:text-red-300"
                    >
                      borrar
                    </button>
                    {isRunning ? (
                      <button
                        ...
                        className="rounded-control px-1.5 py-0.5 text-[12px] text-red-400 transition-colors hover:text-red-200"
                      >
                        detener
                      </button>
                    ) : null}
